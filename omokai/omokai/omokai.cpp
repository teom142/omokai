...
void omokai::set_ai_xy(omok_board& bo)
{
	...
	for (int i = 0; i < MAX_SIZE; i++)
	{
		...
		{
			if (val_board[ai_x][ai_y] < val_board[i][j] && bo.main_board[i][j] == 0)
			{
				ai_x = i;
				ai_y = j;
			}
...
int omokai::close_2(int x, int y, omok_board& bo)
{
	...
	if (is_safe(y + 1) && is_safe(x + 1) && is_safe(x + 2) && is_safe(y + 2))
		return 0;
	if (bo.main_board[x][y + 1] == 1)
	{
		if (is_safe_close(x, y - 1, bo) != is_safe_close(x, y + 2, bo))
		{
			if (!is_safe_close(x, y - 1, bo))
				val_board[x][y - 1] = 30;
			else if (!is_safe_close(x, y + 2, bo))
				val_board[x][y + 2] = 30;
		}
	}
	for (int i = -1; i <= 1; i++)
	{
		if (bo.main_board[x + 1][y - i] == 1)
		{
			if (is_safe_close(x - 1, y + i, bo) != is_safe_close(x + 2, y + i * -2, bo))
			{
				if (!is_safe_close(x - 1, y + i, bo))
					val_board[x - 1][y + i] = 30;
				else if (!is_safe_close(x + 2, y + i * -2, bo))
